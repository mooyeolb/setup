...
do_install() {
	...
	*)
	...
	# nvm	
	$sh_c "wget -qO- https://raw.githubusercontent.com/nvm-sh/nvm/v0.38.0/install.sh | /bin/bash"

	# mambaforge
	mambaforge_home=/home/${user}/.share/mambaforge
	$sh_c "wget -N -P /tmp/ https://github.com/conda-forge/miniforge/releases/latest/download/Mambaforge-$(uname)-$(uname -m).sh"
	$sh_c "bash /tmp/Mambaforge-$(uname)-$(uname -m).sh -bfs -p $mambaforge_home"
	$sh_c "rm /tmp/Mambaforge-$(uname)-$(uname -m).sh"
